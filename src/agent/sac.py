...
class SAC(object):
    ...
    def sample_action(self, obs):
        if obs.shape[-1] != self.image_size:
            obs = center_crop_image(obs, self.image_size)
        with torch.no_grad():
            obs = torch.FloatTensor(obs).to(self.device) ##added.copy here |removed copy again
            obs = obs.unsqueeze(0)
    ...
    def update_critic(self, obs, action, reward, next_obs, not_done, L, step):
        with torch.no_grad():
            ...
            if step % self.log_interval == 0:
                L.log('train_critic/target_q1', target_Q1.mean(), step)
                L.log('train_critic/target_q2', target_Q2.mean(), step)
